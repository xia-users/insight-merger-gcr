...
init-users:
	gcloud iam service-accounts create gcr-xia-merger \
		--display-name "Cloud Run Insight Cleaner";

init-roles:
	@PROJECT_ID=$(shell gcloud config list --format 'value(core.project)'); \
	gcloud projects add-iam-policy-binding $${PROJECT_ID} \
		--member=serviceAccount:gcr-xia-merger@$${PROJECT_ID}.iam.gserviceaccount.com \
...
deploy:
	...
	gcloud run deploy xia-merger \
		--image gcr.io/$${PROJECT_ID}/${{xia.serivce-name}} \
    	--service-account gcr-xia-merger@$${PROJECT_ID}.iam.gserviceaccount.com \
